...
CHANNELS = 1
HARDWARE_IN_RATE = 48000#gemini ecpects 16000Hz PCM -> Then downsample audio_array[::3]->48000/3=16000
HARDWARE_OUT_RATE = 24000#Gemini native audio returns 24000 Hz PCM
CHUNK = 1024
...
class SinhalaBot:
    ...
    async def listen_mic(self):
        stream = pya.open(
            ...
            input=True,
            #input_device_index=1, test to choose default input device 
    ...
    async def play_speaker(self):
        stream = pya.open(
            ...
            output=True,
            #output_device_index =1, test to choose default output device
        )
...
if __name__ == "__main__":
    ...
    # Run AI bot in background thread
    ...
    try:
        video_player.run()   # ? RUN THIS IN MAIN THREAD
